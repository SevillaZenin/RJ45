// RJ45 PRO Lab
// - T568A/T568B
// - Directo (A-A / B-B) y Cruzado (A-B)
// - Validación por orden y por pares (1-2, 3-6, 4-5, 7-8)
// - Niveles: Tutorial -> Examen
// - Timer, score, streak, attempts
// - Sounds with WebAudio (no external files)
...
let state = {
  cableMode: "straight", // "straight" | "crossover"
  baseStd: "B",          // base standard selection
  level: 1,
  score: 0,
  streak: 0,
  attempts: 0,
  ...
  sounds: true,
  pairTest: true,
  hintUsedThisRound: 0,  // for limiting hints
};
...
function standardsForEnds(){
  // returns {A:"A|B", B:"A|B" or null if hidden}
  if (state.cableMode === "straight"){
    return { A: state.baseStd, B: state.baseStd };
  }
  // crossover: A is base, B is opposite
...
function buildPalettes(){
  ...
  // Always create a shuffled set of 8 per end
  const namesA = [...STANDARD.A, ...STANDARD.B]; // pool? We'll use only 8 needed but shuffled strongly
  // Better: always use the 8 wire names, but shuffle. (same 8 used in both standards)
...
function expectedForEnd(end){
  const ends = standardsForEnds();
  const std = ends[end]; // "A" or "B"
  return STANDARD[std];
}

function markSlots(end, expected, actual){
  // mark each pin as ok/bad (only tutorial level)
  const L = activeLevel();
...
function pairValidation(end, expected){
  // Validates that each pair has correct *colors in those pins*.
  // We check exact match on both pins.
...
function computeResult(){
  ...
  for (const end of endsNeeded){
    ...
    if (state.pairTest){
      ...
      if (!pairRes.ok){
        // Keep pair errors (if already order incorrect, this reinforces)
...
function addScore(success){
  ...
  if (success){
    state.streak += 1;
    // score formula: base + speed bonus + streak bonus
    const base = (state.cableMode === "crossover") ? 220 : 140;
    const speed = Math.max(0, Math.floor(120 - seconds)); // faster => more
...
function hint(){
  ...
  // Reveal one correct pin placement suggestion (not auto-place; just highlight expected color name)
  // We'll find first empty slot across needed ends and show expected.
  const endsNeeded = (state.cableMode === "crossover") ? ["A","B"] : ["A"];
  for (const end of endsNeeded){
    const expected = expectedForEnd(end);
    const actual = orderForEnd(end);
    ...
    if (idx !== -1){
      ...
      // brief visual pulse on that slot
...
function showAnswer(){
  ...
  // Auto-place correct order for ends enabled.
  resetSlots();

  // rebuild palettes first to ensure wire elements exist
  buildPalettes();
  ...
  for (const end of endsNeeded){
    ...
    expected.forEach((name, idx) => {
      // find a wire in palette matching name and end
...
function verify(){
  ...
  // in level 1: show detailed problems; in others: shorter
